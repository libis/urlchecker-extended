...
func (c SlackClient) SendMessage(messages []Message) {
	...
	for _, message := range messages {
		msg := fmt.Sprintf("Status: %d, URL: %s, Message: %s", message.Status, message.Url, message.Message)
		formattedMessages = append(formattedMessages, msg)
	}

	// concatenate all formatted messages
	allMessages := fmt.Sprintf("Repository: %s, Messages: [%s]", repo, strings.Join(formattedMessages, "; "))
